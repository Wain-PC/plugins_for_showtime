...
(function(plugin) {
    ...
    //settings
    var service = plugin.createService("www.AniMult.tv", PREFIX + ":start", "video", true, plugin.path + "images/logo.png");
    var settings = plugin.createSettings("www.AniMult.tv", plugin.path + "images/logo.png", "OnLine Video Site");
    ...
    plugin.addURI(PREFIX + ":start", function(page) {
        ...
        page.metadata.title = "www.AniMult.tv";
        page.type = "directory";
        page.contents = "list";
	...
	if ((video[1].indexOf('xml') !== -1)){
	    var pl = showtime.httpGet('http://animult.tv' + video[1]).toString();
		...
		page.appendItem(PREFIX + ":play:" + escape(video[1]) +":"+ escape(title[1]+"\n"+ stitle[1]) , "video",{
		    title: unescape(title[1])+"/"+ stitle[1],
		    icon : metadata.icon,
	...
	var cat = getValue(html, '<div class="block"><h1>Сериалы по жанрам</h1>', '</div>');
	    cat = cat.split('<br />');

	for (var i in cat) {
            var match = /http:\/\/animult.tv(.*\/)".*">(.*)<\/a>/.exec(cat[i]);
            if (match)
	...
	var entries = [];
	    ...
	    for (var i = 1; i < items.length; i++) {
	        var url_title = /<h1> <a href="http:\/\/animult.tv([^"]+)">([^<]+)/.exec(items[i]);
